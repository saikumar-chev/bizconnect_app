...
// IMPORTANT: This ID is for example purposes. Replace with your actual Google Client ID.
const GOOGLE_CLIENT_ID = "933812992581-ab273p6lc3tle0f2e5kdtbott0q30b3o.apps.googleusercontent.com";
...
const App: React.FC = () => {
  ...
  const onLogin = useCallback(async (credentialResponse: any) => {
    ...
    // After migration, the stable identifier to find a user is their email, not the Google 'sub' ID.
    const { data: existingUserData, error: fetchError } = await supabase
      .from('users')
      .select('*')
      .eq('email', email) // Query by email to find the user record.
      .single();

    if (fetchError && fetchError.code !== 'PGRST116') { // PGRST116: "exact one row not found"
        setError(`Failed to log in: ${fetchError.message}`);
        return;
    }
    
    // Upsert the user data based on their email.
    // Supabase will either update the existing user or create a new one if the email doesn't exist.
    // We specify the email as the `onConflict` column.
    const { data: upsertedUserData, error: upsertError } = await supabase.from('users').upsert({
      // id is now generated by the database, so we don't provide it for new users.
      // We only provide it if we are updating an existing user.
      id: existingUserData?.id, 
      name: name,
      email: email,
      avatarurl: picture,
      bio: existingUserData?.bio || '', // Preserve existing bio on login
    }, { onConflict: 'email' }).select().single();

    if (upsertError) {
        setError(`Failed to save user data: ${upsertError.message}`);
        return;
    }

    // The user object in our app state MUST use the UUID from the database.
    const loggedInUser: User = {
        userId: upsertedUserData.id, // Use the UUID from the database response.
  ...
  }, []); // Dependencies are intentionally empty to create a stable function reference for Google's callback
